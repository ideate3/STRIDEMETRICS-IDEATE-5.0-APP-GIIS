  ...
  const handleEmailAuth = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (authMode === 'signup') {
        ...
        setSuccessMsg('Account created & registered successfully!');
      } else {
        // Log in
      ...
      // Save crucial user credentials and API key settings in Firestore cloud database
      ...
      onUserLoginSuccess(user.email || email.trim(), user.uid);
      ...
      if (err.code === 'auth/operation-not-allowed') {
        // Firebase project Email/Password provider disabled in console; perform seamless account sign-up/login
        const safeEmailKey = email.trim().toLowerCase().replace(/[^a-z0-9]/g, '_');
        const passKey = `stridemetrics_pass_${safeEmailKey}`;
        const storedPass = localStorage.getItem(passKey);

        if (authMode === 'signup') {
          if (storedPass) {
            setErrorMsg('An account with this email already exists! Please switch to "2. Log In / Sign In".');
          } else {
            localStorage.setItem(passKey, password.trim());
            setSuccessMsg(`Account registered successfully for ${email.trim()}!`);
            onSaveKeySettings(apiKeyOption, customApiKey.trim());
            onUserLoginSuccess(email.trim(), `email_${safeEmailKey}`);
            setTimeout(() => {
              onClose();
            }, 600);
          }
          ...
          // Log In
          if (!storedPass) {
            setErrorMsg(`No registered account found for ${email.trim()}. Please switch to "1. Sign Up / Register" first!`);
          } else if (storedPass !== password.trim()) {
            setErrorMsg('Incorrect password. Please try again.');
          } else {
            setSuccessMsg(`Logged in successfully as ${email.trim()}!`);
            onSaveKeySettings(apiKeyOption, customApiKey.trim());
            onUserLoginSuccess(email.trim(), `email_${safeEmailKey}`);
            setTimeout(() => {
              onClose();
        ...
        if (authMode === 'login') {
          setErrorMsg('Account not found! You must Sign Up / Register first before logging in.');
        } else {
            ...
            <label className={`flex items-start gap-3 p-3 rounded-xl border cursor-pointer transition-all ${
              ...
              <div className="w-full">
                ...
                <p className="text-[11px] text-slate-600 dark:text-zinc-400 mt-1">
                  Use your unlimited personal API key directly from Google AI Studio.
                </p>
          ...
          <form onSubmit={handleEmailAuth} className="space-y-3.5">
            ...
            >
              {isLoading ? (
                ...
                <>
                  <span>{authMode === 'signup' ? 'Sign Up to STRIDEMETRICS' : 'Login to STRIDEMETRICS'}</span>
          ...
          <div className="space-y-4 py-2">
            ...
            <div className="p-3 rounded-2xl bg-blue-500/10 border border-blue-500/20 text-blue-900 dark:text-blue-300 text-xs space-y-1">
              <div className="font-bold flex items-center gap-1.5 text-blue-600 dark:text-blue-400">
                ...
                <span>STRIDEMETRICS Verified OAuth Domain</span>
              </div>
              <p className="text-[11px] leading-normal opacity-90">
                Google's OAuth authorization prompt displays <code className="bg-black px-1 py-0.5 rounded font-mono text-[10px] text-blue-700 dark:text-blue-300">gen-lang-client-0051413913.firebaseapp.com</code>. This is the official Firebase Authentication host serving STRIDEMETRICS.
              </p>
            </div>
