...
import { Send, Image as ImageIcon, Utensils, Zap, Trash2, Cloud, HardDrive, ShieldCheck } from 'lucide-react';
...
export const CoachChat: React.FC<CoachChatProps> = ({
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-4rem)] md:h-[calc(100vh-5rem)] max-w-5xl mx-auto px-3 sm:px-6 py-3 sm:py-4">
      ...
      <div className="flex-1 overflow-y-auto space-y-4 pr-1 sm:pr-2">
        ...
        {(() => {
          ...
          return (
            <motion.div
              ...
              className="squircle-card liquid-glass p-4 sm:p-6 relative overflow-hidden"
            >
              <div className="flex items-start gap-3 sm:gap-4">
                <div className="flex h-11 w-11 sm:h-12 sm:w-12 shrink-0 items-center justify-center rounded-2xl bg-emerald-500 text-slate-950 font-black text-lg font-display">
                  {coachInitial}
                </div>
                <div className="flex-1">
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <div className="flex flex-wrap items-center gap-2">
                      <h2 className="text-base sm:text-lg font-bold text-slate-900 dark:text-white font-display">{displayCoachName}</h2>
                      <span className="hidden sm:inline text-slate-600 dark:text-zinc-400 text-xs font-normal">— Active Assistant</span>
                      <span className="rounded-full bg-emerald-500/15 px-2.5 py-0.5 text-[10px] font-mono font-bold text-emerald-700 dark:text-emerald-400 border border-emerald-500/30">
                    ...
                    <div className="flex items-center gap-2">
                      {/* Storage Mode Badge */}
                      {isCloudStorage ? (
                        <span className="flex items-center gap-1 rounded-full bg-emerald-500/10 border border-emerald-500/25 px-2.5 py-1 text-[10px] font-bold text-emerald-600 dark:text-emerald-400" title="Chats are saved to encrypted Firestore Cloud database">
                          <Cloud className="h-3 w-3 text-emerald-500" />
                          <span className="hidden sm:inline">Cloud Secured</span>
                        </span>
                      ) : (
                        <span className="flex items-center gap-1 rounded-full bg-amber-500/10 border border-amber-500/25 px-2.5 py-1 text-[10px] font-bold text-amber-600 dark:text-amber-400" title="Chats are saved solely in this local PC browser">
                          <HardDrive className="h-3 w-3 text-amber-500" />
                          <span className="hidden sm:inline">Local PC</span>
                        </span>
                      )}
                      ...
                      {onClearChatHistory && (
                        <button
                          ...
                          className="flex items-center gap-1 rounded-xl bg-slate-200 dark:bg-white/10 hover:bg-rose-500/20 hover:text-rose-500 px-2.5 py-1 text-xs text-slate-700 dark:text-zinc-300 transition-colors"
                          title="Clear Chat History"
                        >
                          <Trash2 className="h-3.5 w-3.5" />
                          <span className="hidden md:inline font-medium">Clear Chat</span>
                        </button>
                      )}
                    </div>
                  </div>
                  <p className="mt-1.5 text-xs sm:text-sm text-slate-800 dark:text-zinc-300 leading-relaxed">
                    I'm your dedicated health and performance coach. Ask me about custom training routines, macro breakdowns, form checks, or rapid recovery strategy.
                  </p>
                  
                  {/* Quick Prompt Chips */}
                  <div className="mt-3.5 flex overflow-x-auto sm:flex-wrap gap-2 pb-1 no-scrollbar">
                    {quickPrompts.map((qp, idx) => (
                      <button
                        key={idx}
                        onClick={() => setInputText(qp.prompt)}
                        className="shrink-0 flex items-center gap-1.5 squircle-btn border border-slate-300 dark:border-white/10 bg-slate-100 dark:bg-white/[0.05] backdrop-blur-md px-3 py-1.5 text-xs text-slate-800 dark:text-zinc-300 hover:bg-emerald-500/15 hover:text-emerald-700 dark:hover:text-emerald-400 hover:border-emerald-500/40 transition-all font-medium active:scale-95"
                      >
        ...
        <AnimatePresence initial={false}>
          ...
          {isLoading && (
            ...
            >
              <div className="flex h-8 w-8 sm:h-9 sm:w-9 items-center justify-center rounded-xl bg-emerald-500 text-slate-950 font-black text-xs sm:text-sm font-display">
                J
              </div>
              <div className="squircle-card liquid-glass border-l-2 border-l-emerald-400 px-3.5 py-2.5 text-xs text-emerald-400 flex items-center gap-2">
                <span>Jason is typing...</span>
